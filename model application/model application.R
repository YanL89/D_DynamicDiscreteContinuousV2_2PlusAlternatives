...
dyn = list()
for(t in 1:time_sim){
  dyn[[t]] = read.table(paste(paste("dyn", t, sep = ""), "txt", sep = "."), sep = " ", head = T)
}
...
write.table(choice, file = "choice_application.txt", row.names=FALSE, col.names=FALSE)
#save continuous dependent variable - "miles"
miles_name = paste("vmt", seq(1, spec$nTime, 1), sep="")
miles = rbind(miles_name, Y)
write.table(miles, file = "miles_application.txt", row.names=FALSE, col.names=FALSE)
